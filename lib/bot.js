...
var QQBot = function () {
    ...
    self._login_by_qrcode = function(){
        ...
        self.client.get(init_url,function(body){
            html += body;
        	let appid = html.match(/<input type="hidden" name="aid" value="(\d+)"/)[0],
                sign = html.match(/g_login_sig=encodeURIComponent\("(.*?)"\)/)[0],
                js_ver = html.match(/g_pt_version=encodeURIComponent\("(\d+)"\)/)[0],
                mibao_css = html.match(/g_mibao_css=encodeURIComponent\("(.+?)"\)/)[0];
            console.log(appid);
            down_Qr(appid);
        });
    }
